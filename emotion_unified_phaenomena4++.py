    ...
    <script>
        ...
        async function initGesture() {
            try {
                ...
                // Check if server camera is working
                const testResp = await fetch('/gesture_frame');
                if (!testResp.ok || testResp.status === 204) {
                    // No camera — show picker
                    await showServerCameraPicker();
                    return;
                }
        ...
        async function showServerCameraPicker() {
            try {
                ...
                cameras.forEach(cam => {
                    ...
                    opt.value = cam.index;
                    opt.textContent = `[${cam.index}] ${cam.backend} ${cam.width}×${cam.height}`;
                ...
                btn.onclick = async () => {
                    btn.textContent = '...';
                    const r = await fetch('/api/switch_camera/' + select.value, { method: 'POST' });
        ...
        function startPoseLoop() {
            ...
            gestureOverlay.ondblclick = () => showServerCameraPicker();
        ...
        function retrain() {
            fetch('/retrain', { method: 'POST' })
                ...
                .then(d => {
                    ...
                    if (d.status === 'retrained') alert('Retrained: ' + d.message);
