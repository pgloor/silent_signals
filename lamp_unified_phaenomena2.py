...
def training_lamp_controller():
    while True:
        if state.mode == 'training' and shelly_plug and (shelly_plug.connected or (shelly_plug._check_connection() or shelly_plug.connected)):
                ...
                while True:
                    try:
                        ...
                        if data.get("type") == "data":
                            ...
                            # Sync lamp state from ESP32 physical button
                            # Only use ESP32 lampState when no Shelly plug is connected
                            # (Shelly is the authority when present)
                            if "lampState" in data and (not shelly_plug or not shelly_plug.connected):
                                esp32_lamp = data["lampState"]
                                if esp32_lamp != state.lamp_on:
                                    state.lamp_on = esp32_lamp
                                    state.lamp_toggle_time = current_time
                                    print(f"🔘 Lamp state synced from ESP32: {'ON' if esp32_lamp else 'OFF'}")
...
<body>
    ...
    <script>
        ...
        let _sigW=0,_sigH=0;
        function resizeSig(){
            const r=sigCanvas.parentElement.getBoundingClientRect();
            const w=Math.round(r.width)||window.innerWidth;
            const h=Math.max(Math.round(r.height)-12,20);
            if(w!==_sigW||h!==_sigH){_sigW=w;_sigH=h;sigCanvas.width=w;sigCanvas.height=h;}
        }
        function drawSignal(data,isLampOn) {
            if(_sigW===0||_sigH===0)resizeSig();const w=_sigW,h=_sigH;sigCtx.clearRect(0,0,w,h);
        ...
        function retrain(){
            fetch('/retrain',{method:'POST'})
                ...
                .then(d=>{
                    ...
                    else if(d.message) { const fb=document.getElementById('gestureFeedback'); if(fb){fb.textContent=d.message;fb.className='gesture-feedback active';setTimeout(()=>fb.className='gesture-feedback',3000);} }
                })
        ...
        window.addEventListener('resize',()=>{_sigW=0;_sigH=0;resizeSig();});
...
@app.route('/shelly_status')
def shelly_status():
    global shelly_plug
    if not shelly_plug:
        shelly_plug = ShellyPlug(SHELLY_IP)
    shelly_plug._check_connection()
    return jsonify({
        'ip': SHELLY_IP,
        'connected': shelly_plug.connected,
        'lamp_on': shelly_plug.last_status
    })
...
def toggle_lamp():
    ...
    if not shelly_plug:
        shelly_plug = ShellyPlug(SHELLY_IP)
    if not shelly_plug.connected:
        shelly_plug._check_connection()  # retry
    if not shelly_plug.connected:
        return jsonify({'status': 'error', 'error': 'Plug not reachable'})
...
def set_lamp():
    ...
    # Always send command to Shelly — don't skip based on state.lamp_on
    # because ESP32 lampState sync may have overwritten it incorrectly
    success = shelly_plug.turn_on() if new_state else shelly_plug.turn_off()
    if success:
        state.lamp_on = new_state
        state.lamp_toggle_time = time.time()
        state.training_phase = 'waiting'
        print(f"💡 Lamp set to {'ON' if new_state else 'OFF'} via browser")
        return jsonify({'status': 'ok', 'lamp_on': state.lamp_on})
    return jsonify({'status': 'error'})
...
def retrain():
    ...
    if n_samples < MIN_PAIRS_FOR_RETRAIN:
        print(f"🔄 Retrain requested but only {n_samples} samples — bootstrapping from WAV files...")
        success = bootstrap_lamp_from_wav_files(max_age_minutes=1440)
        if success:
            return jsonify({'status': 'ok', 'message': f'Retrained from WAV files ({len(state.model.training_data)} samples, {state.model.accuracy_estimate:.1%})'})
        else:
            return jsonify({'status': 'error', 'message': f'Not enough data: {n_samples} samples, need {MIN_PAIRS_FOR_RETRAIN}. Collect more training pairs first.'})
...
def bootstrap_lamp_from_wav_files(max_age_minutes=60):
    ...
    for f in wav_files:
        ...
        if age_min > max_age_minutes:
            continue
        if '_lamp_on_' in f.name:
            on_files.append(f)
        elif '_lamp_off_' in f.name:
            off_files.append(f)
    
    total = len(on_files) + len(off_files)
    print(f"📂 Found {total} fresh WAV files (<{max_age_minutes}min): {len(on_files)} lamp_on, {len(off_files)} lamp_off")
    
    if total < MIN_PAIRS_FOR_RETRAIN or len(on_files) < 3 or len(off_files) < 3:
        print(f"   Not enough for training (need {MIN_PAIRS_FOR_RETRAIN})")
        return False
    
    loaded = 0
    for f, label in [(of, 1) for of in on_files] + [(ff, 0) for ff in off_files]:
        try:
            ...
            if len(voltage) >= WINDOW_SIZE:
                features = extract_features(voltage[:WINDOW_SIZE], SAMPLE_RATE)
                if features:
                    arr = features_to_array(features)
                    if arr is not None:
                        state.model.training_data.append((arr, label))
                        loaded += 1
        except Exception as e:
            print(f"   ⚠️  Error loading {f.name}: {e}")
    
    state.model.training_count = loaded
    n_on = sum(1 for _, l in state.model.training_data if l == 1)
    n_off = loaded - n_on
    print(f"   ✅ Loaded {loaded} samples from WAV files ({n_on} lamp_on, {n_off} lamp_off)")
    state.lamp_on_count = n_on
    state.lamp_off_count = n_off
...
if __name__ == '__main__':
    ...
    if shelly_plug.connected:
        ...
        print("⚠️ Shelly plug not reachable at startup — will retry on use")
        # Keep shelly_plug object so routes can retry; don't set to None
    ...
    if loaded and age < MODEL_MAX_AGE_MINUTES:
        ...
        if bootstrap_lamp_from_wav_files(max_age_minutes=1440):
            state.mode = 'exhibition'
