...
if not USE_HSEMOTION:
    try:
        ...
        device = 'cuda' if torch.cuda.is_available() else 'cpu'
        MTCNN_DETECTOR = MTCNN(keep_all=False, device=device, min_face_size=60)
        HSEMOTION_MODEL = HSEmotionRecognizer(model_name='enet_b0_8_va_mtl', device=device)
...
# Camera index — set to -1 for auto-detect, or 0/1/2/3 to pick a specific camera
# Run with --list-cameras to see all available cameras and their indices
CAMERA_INDEX = 1
CAMERA_WIDTH = 640
CAMERA_HEIGHT = 480
...
def map_valence_to_category(scores):
    """
    Convert va_mtl scores array to (category, confidence) using valence.
    scores: array of 10 floats — [0..7] emotion probs, [8] valence, [9] arousal

    Replaces label-based mapping that misclassifies focused/working mood
    (contempt, disgust) as negative. Valence correctly stays near 0 for
    neutral working states, only going negative when genuinely sad/distressed.

    Returns:
        category   : 'positive' | 'negative' | 'neutral'
        confidence : 0.0-1.0  maps directly to the Freude/Trauer scale bar
                     (0.0 = very sad, 0.5 = neutral, 1.0 = very happy)
    """
    scores_arr = np.array(scores) if not isinstance(scores, np.ndarray) else scores

    if len(scores_arr) >= 9:
        valence = float(scores_arr[8])   # raw valence from va_mtl model
    else:
        # Fallback: estimate valence from emotion probs if model didn't supply it
        anger, contempt, disgust, fear, happiness, neutral, sadness, surprise = scores_arr[:8]
        valence = float(happiness + neutral * 0.5 + surprise * 0.3
                        - contempt * 0.4 - disgust * 0.4 - sadness * 0.6
                        - anger * 0.5 - fear * 0.3)
        valence = max(-1.0, min(1.0, valence))

    # Map valence [-1,+1] -> confidence [0,1] (0.5 = neutral midpoint)
    confidence = (valence + 1.0) / 2.0

    # Dead zone around neutral to avoid flickering
    DEAD_ZONE = 0.12
    if valence > DEAD_ZONE:
        category = 'positive'
    elif valence < -DEAD_ZONE:
        category = 'negative'
    else:
        category = 'neutral'
...
def list_cameras(max_index=5):
    """Scan camera indices and return list of (index, name, width, height)."""
    cameras = []
...
def print_cameras():
    ...
    print("\n📷 Scanning cameras (indices 0–4)...")
    cameras = list_cameras()
    if not cameras:
        print("   ❌ No cameras found")
    else:
        for idx, backend, w, h in cameras:
            print(f"   [{idx}] {backend} — {w}×{h}")
        print(f"\n   Set CAMERA_INDEX to the desired index, or use -1 for auto-detect")
    return cameras
...
class CameraHandler:
    def __init__(self):
        ...
        # ── Valence calibration (ported from emoti_plant / wellbeing_app) ──
        self._calib_buf   = []          # raw valence samples during baseline phase
        self._calib_n     = 12          # samples needed (~60s at 5s interval)
        self._baseline_v  = None        # mean raw valence of resting face
        self._ema_v       = None        # exponentially-smoothed calibrated valence
        self._EMA_FAST    = 0.35        # fast track for positive moves
        self._EMA_SLOW    = 0.12        # slow decay for negative moves
        self._last_proc   = 0.0         # throttle: one calibration sample per 5s
    
    def start(self):
        """Start camera. If CAMERA_INDEX == -1, auto-detect (prefer external cameras)."""
        if CAMERA_INDEX >= 0:
            return self._try_open(CAMERA_INDEX)
        
        # Auto-detect: scan and prefer higher indices (external cameras)
        cameras = list_cameras()
        if not cameras:
            return False
        
        for idx, backend, w, h in cameras:
            print(f"   📷 Found camera [{idx}] {backend} {w}×{h}")
        
        # Prefer the highest index (external/monitor cameras usually come after built-in)
        preferred = cameras[-1][0] if len(cameras) > 1 else cameras[0][0]
        print(f"   → Auto-selecting camera [{preferred}]" +
              (" (external)" if preferred > 0 else " (built-in)"))
        return self._try_open(preferred)
    
    def _try_open(self, index):
        try:
            ...
            if self.cap.isOpened():
                ...
                if ret:
                    ...
                    print(f"   ✅ Camera [{index}] opened — {actual_w}×{actual_h}")
                return ret
            return False
        except:
            return False
    ...
    def _detect_emotion_hsemotion(self, frame):
        try:
            ...
            # ── Calibrated valence pipeline ─────────────────────────────
            import time as _time
            raw_v = float(scores[8]) if len(scores) > 8 else 0.0
            now   = _time.time()

            # Throttle: collect one sample every 5 s
            if now - self._last_proc >= 5.0:
                self._last_proc = now
                if self._baseline_v is None:
                    # Baseline phase: accumulate resting-face samples
                    self._calib_buf.append(raw_v)
                    if len(self._calib_buf) >= self._calib_n:
                        self._baseline_v = float(sum(self._calib_buf) / len(self._calib_buf))
                        print(f"  📐 [Phänomena] Valence baseline calibrated: {self._baseline_v:.3f}")
                else:
                    # Calibrate and apply asymmetric EMA
                    v_dm = raw_v - self._baseline_v           # de-meaned, centred on 0
                    if self._ema_v is None:
                        self._ema_v = v_dm
                    else:
                        alpha = self._EMA_FAST if v_dm > self._ema_v else self._EMA_SLOW
                        self._ema_v = alpha * v_dm + (1 - alpha) * self._ema_v

            # Use calibrated valence when available, else raw
            if self._ema_v is not None:
                cal_v = max(-1.0, min(1.0, self._ema_v))
            elif self._baseline_v is not None:
                cal_v = max(-1.0, min(1.0, raw_v - self._baseline_v))
            else:
                cal_v = raw_v   # still calibrating — use raw

            # Override scores[8] with calibrated valence for map_valence_to_category
            scores_cal = list(scores)
            scores_cal[8] = cal_v
            # ────────────────────────────────────────────────────────────

            category, adj_confidence = map_valence_to_category(scores_cal)
...
<head>
    ...
    <style>
        @font-face { font-family: 'TownCountry'; src: url('/fonts/TownandCountryJNL.otf') format('opentype'); }
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .container { display: flex; flex-direction: column; height: 100vh; padding: 20px 30px; }
        
        /* Header */
        .header { text-align: center; margin-bottom: 20px; }
        .exhibit-title { font-family: 'TownCountry', serif; font-size: 2.6em; font-weight: 400; letter-spacing: 0.05em; color: var(--green); margin-bottom: 6px; }
        .exhibit-subtitle { font-size: 1.3em; font-weight: 400; color: rgba(255,255,255,0.85); }
        ...
        .main-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 20px; flex: 1; min-height: 0; max-height: 500px; }
        
        .panel { background: var(--panel-bg); border-radius: 16px; padding: 20px; display: flex; flex-direction: column; align-items: center; }
        .panel-title { font-size: 1.1em; font-weight: 600; color: rgba(255,255,255,0.8); margin-bottom: 15px; text-align: center; }
        
        /* AI Panel - Camera */
        .camera-container { flex: 0 1 auto; width: 100%; max-width: 360px; max-height: 280px; aspect-ratio: 4/3; border-radius: 12px; overflow: hidden; background: #000; }
        .camera-container img { width: 100%; height: 100%; object-fit: cover; }
        
        .face-emotion { margin-top: 15px; padding: 10px 16px; border-radius: 10px; background: rgba(0,0,0,0.3); text-align: center; font-size: 1.2em; font-weight: 600; }
        .face-emotion.positive { color: var(--green); }
        .face-emotion.negative { color: var(--pink); }
        .face-emotion.neutral { color: rgba(255,255,255,0.6); }
        
        /* Plant Panel */
        .plant-content { flex: 1; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 10px; width: 100%; }
        
        .video-container { width: 200px; height: 200px; border-radius: 50%; overflow: hidden; border: 4px solid var(--green); box-shadow: 0 0 40px rgba(28,255,168,0.3); transition: all 0.4s; flex-shrink: 0; }
        ...
        .plant-sensing { font-size: 1.1em; color: rgba(255,255,255,0.7); margin-top: 10px; }
        .plant-response { font-size: 1.8em; font-weight: 700; text-align: center; }
        ...
        /* Confidence Scale */
        .confidence-scale { width: 100%; max-width: 300px; margin-top: 15px; }
        .scale-labels { display: flex; justify-content: space-between; font-size: 0.75em; color: rgba(255,255,255,0.6); margin-bottom: 6px; }
        .scale-labels span { text-align: center; flex: 1; }
        .scale-bar { height: 8px; background: linear-gradient(90deg, var(--pink) 0%, rgba(255,255,255,0.2) 50%, var(--green) 100%); border-radius: 4px; position: relative; }
        .scale-marker { position: absolute; top: -4px; width: 4px; height: 16px; background: white; border-radius: 2px; transform: translateX(-50%); transition: left 0.3s; box-shadow: 0 0 8px rgba(255,255,255,0.8); }
        .scale-ticks { display: flex; justify-content: space-between; margin-top: 4px; padding: 0 2px; }
        .scale-ticks span { width: 1px; height: 6px; background: rgba(255,255,255,0.3); }
        
        /* Bottom section */
        .bottom-section { margin-top: 15px; display: flex; flex-direction: column; gap: 12px; }
        
        /* Collection indicator */
        .collection-indicator { display: none; padding: 12px 20px; background: rgba(28,255,168,0.15); border: 2px solid var(--green); border-radius: 10px; text-align: center; animation: pulse-border 1s infinite; }
        ...
        .collection-indicator .progress-bar { width: 200px; height: 6px; background: rgba(255,255,255,0.1); border-radius: 3px; margin: 8px auto 0; overflow: hidden; }
        .collection-indicator .progress-fill { height: 100%; background: var(--green); transition: width 0.2s; }
        
        /* Training stats */
        .training-stats { display: none; justify-content: center; gap: 30px; padding: 10px; background: rgba(0,0,0,0.2); border-radius: 10px; }
        ...
        /* Signal */
        .signal-container { height: 70px; background: rgba(255,255,255,0.03); border-radius: 10px; padding: 8px 12px; border: 1px solid rgba(28,255,168,0.15); }
        .signal-title { font-size: 0.65em; color: rgba(255,255,255,0.4); text-transform: uppercase; letter-spacing: 1.5px; margin-bottom: 4px; }
        #signalCanvas { width: 100%; height: calc(100% - 16px); }
        
        /* ===== CORNER OVERLAY GESTURE CAMERA ===== */
        .gesture-overlay { position: fixed; bottom: 15px; left: 15px; width: 160px; height: 120px; border-radius: 10px; overflow: hidden; border: 2px solid rgba(28,255,168,0.3); background: rgba(0,0,0,0.5); z-index: 90; transition: all 0.3s; }
        .gesture-overlay:hover { transform: scale(1.05); border-color: var(--green); }
        ...
        .gesture-feedback { position: fixed; bottom: 145px; left: 15px; width: 160px; text-align: center; font-size: 1.3em; font-weight: 700; color: var(--green); text-shadow: 0 0 15px var(--green); opacity: 0; pointer-events: none; z-index: 95; }
        ...
        .language-float { position: fixed; bottom: 20px; left: 190px; display: flex; gap: 6px; z-index: 90; }
        .lang-btn { padding: 6px 14px; border-radius: 16px; font-size: 0.9em; font-weight: 600; background: rgba(32,24,68,0.9); border: 2px solid rgba(255,255,255,0.2); color: rgba(255,255,255,0.5); cursor: pointer; transition: all 0.3s; font-family: inherit; }
        ...
        .swipe-hint { position: fixed; bottom: 140px; left: 15px; width: 160px; text-align: center; font-size: 0.65em; color: rgba(255,255,255,0.35); }
        
        /* Staff controls */
        .staff-controls { position: fixed; bottom: 10px; right: 10px; display: flex; gap: 5px; opacity: 0.15; transition: opacity 0.3s; z-index: 100; }
                ...
                <div class="plant-content">
                    ...
                    <div class="confidence-scale">
                        <div class="scale-labels">
                            ...
                            <span id="scaleLabel2">Trauer<br>wahrsch.</span>
                            <span id="scaleLabel3">Freude<br>wahrsch.</span>
    ...
    <script>
        ...
        const T = {
            de: { 
                ...
                scale: ['Trauer<br>sicher', 'Trauer<br>wahrsch.', 'Freude<br>wahrsch.', 'Freude<br>sicher'],
            ...
            en: { 
                ...
                scale: ['Sadness<br>certainly', 'Sadness<br>likely', 'Happiness<br>likely', 'Happiness<br>certainly'],
        ...
        const swipeThreshold = 0.9, histLen = 8;
        ...
        async function initGesture() {
            try {
                ...
                const pose = new Pose({ locateFile: f => `/mediapipe_pose/${f}` });
                pose.setOptions({ modelComplexity: 0, smoothLandmarks: true, minDetectionConfidence: 0.5, minTrackingConfidence: 0.5 });
                pose.onResults(onPose);

                async function fetchAndProcess() {
                    try {
                        const resp = await fetch('/gesture_frame');
                    ...
                    requestAnimationFrame(fetchAndProcess);
                }
                fetchAndProcess();
            } catch (e) {
        ...
        function resizeSig() { const c = sigCanvas.parentElement; sigCanvas.width = c.clientWidth - 24; sigCanvas.height = c.clientHeight - 20; }
        ...
        socket.on('update', d => {
            ...
            if(dbg) dbg.textContent = 'conn:'+d.connected+' buf:'+(d.voltages?d.voltages.length:0)+' w:'+sigCanvas.width+'×'+sigCanvas.height;
            ...
            if (d.mode === 'exhibition' && d.prediction) {
                ...
                // Scale: 0% = sadness certainly, 50% = neutral, 100% = happiness certainly
        ...
        window.addEventListener('resize', resizeSig); resizeSig(); initGesture();
...
@app.route('/gesture_frame')
def gesture_frame():
    """Single JPEG snapshot for gesture detection"""
    frame = camera.get_frame()
    if frame:
...
def api_cameras():
    """List available cameras for the picker UI."""
    cameras = list_cameras()
    return jsonify([{'index': idx, 'backend': backend, 'width': w, 'height': h} for idx, backend, w, h in cameras])

@app.route('/api/switch_camera/<int:index>', methods=['POST'])
def api_switch_camera(index):
    """Switch the server-side OpenCV camera to a different index."""
    if camera.cap:
        camera.cap.release()
    if camera._try_open(index):
        state.camera_connected = True
        return jsonify({'status': 'ok', 'index': index})
    else:
        state.camera_connected = False
        return jsonify({'status': 'error', 'message': f'Failed to open camera {index}'})
...
def retrain():
    n_pairs = len(state.model.training_pairs)
    if n_pairs < MIN_PAIRS_FOR_RETRAIN:
        print(f"🔄 Retrain requested but only {n_pairs} pairs — bootstrapping from WAV files...")
        success = bootstrap_emotion_from_wav_files(max_age_minutes=1440)
        if success:
            state.mode = 'exhibition'
            return jsonify({'status': 'retrained', 'message': f'{state.model.accuracy:.1%} accuracy, {len(state.model.training_pairs)} samples'})
        else:
            state.mode = 'training'
            return jsonify({'status': 'error', 'message': f'Not enough data: {n_pairs} pairs. Record more positive/negative samples.'})
    state.mode = 'training'
    result = state.model.force_retrain()
    if result:
        state.mode = 'exhibition'
        return jsonify({'status': 'retrained', 'message': f'{state.model.accuracy:.1%} accuracy, {len(state.model.training_pairs)} samples'})
...
def bootstrap_emotion_from_wav_files(max_age_minutes=60):
    """Scan WAV files in training folder, load fresh ones, and retrain if enough exist."""
    wav_files = sorted(OUTPUT_DIR.glob("plant_emotion_*.wav"))
    if not wav_files:
        return False
    ...
    for f in wav_files:
        ...
        if age_min > max_age_minutes:
            continue
        if '_positive_' in f.name:
            pos_files.append(f)
        elif '_negative_' in f.name:
            neg_files.append(f)
    
    total = len(pos_files) + len(neg_files)
    print(f"📂 Found {total} fresh WAV files (<{max_age_minutes}min): {len(pos_files)} positive, {len(neg_files)} negative")
    ...
    loaded = 0
    for f, label in [(pf, 'positive') for pf in pos_files] + [(nf, 'negative') for nf in neg_files]:
        try:
            ...
            if len(voltage) >= 100:
                features = extract_features(list(voltage))
                if features:
                    state.model.training_pairs.append((features, label))
                    loaded += 1
        except Exception as e:
            print(f"   ⚠️  Error loading {f.name}: {e}")
    
    n_pos = sum(1 for _, l in state.model.training_pairs if l == 'positive')
    n_neg = loaded - n_pos
    print(f"   ✅ Loaded {loaded} samples from WAV files ({n_pos} positive, {n_neg} negative)")
    state.positive_count = n_pos
    state.negative_count = n_neg
    
    if loaded >= MIN_PAIRS_FOR_RETRAIN:
        print("   🔄 Retraining from WAV data...")
        return state.model.train()
    
    return False


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Phaenomena Emotion Perception')
    parser.add_argument('--list-cameras', action='store_true', help='List available cameras and exit')
    parser.add_argument('--camera', type=int, default=None, help='Camera index to use (overrides CAMERA_INDEX)')
    ...
    if camera.start():
        ...
        print("⚠️  Camera not available — use gesture overlay picker to select")
    ...
    if loaded and age < MODEL_MAX_AGE_MINUTES:
        ...
        if bootstrap_emotion_from_wav_files(max_age_minutes=1440):
            state.mode = 'exhibition'
